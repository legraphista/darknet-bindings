...
class DarknetImage {
  ...
  async prepare() {
    ...
    const { w, h, c, data_pointer } = new Promise((res, rej) =>
      this._darknet.letterbox(this.data, w, h, c, (e, data) => {
        if (e) {
          return rej(e);
        }
        return res(data);
...
class Darknet {
  constructor({
    ...
    assert(fs.exists(cfg), 'bad config file location');
    assert(fs.exists(weights), 'bad weights file location');
  ...
  async _rgbBufferToDarknetImage(buffer, { w, h, c }) {
    ...
    const darknetBuffer = new Promise((res, rej) =>
